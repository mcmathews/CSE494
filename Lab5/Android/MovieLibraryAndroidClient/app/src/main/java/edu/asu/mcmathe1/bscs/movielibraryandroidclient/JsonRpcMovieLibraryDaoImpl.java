...
public class JsonRpcMovieLibraryDaoImpl implements MovieLibraryDao {
	...
	protected boolean isSuccessful(JSONObject response) {
		try {
			return response.getBoolean("result");
		} catch (JSONException e) {
			Log.e(getClass().getSimpleName(), "result not in response");
			return false;
		}
	}
	...
	public boolean reset() throws IOException {
		return isSuccessful(makeJsonRpcRequest("reset"));
	}

	@Override
	public boolean save() throws IOException {
		return isSuccessful(makeJsonRpcRequest("save"));
	}

	@Override
	public boolean add(MovieDescription movie) throws IOException {
		return isSuccessful(makeJsonRpcRequest("add", movie.toJsonObject()));
	}

	@Override
	public boolean edit(String title, MovieDescription movie) throws IOException {
		return isSuccessful(makeJsonRpcRequest("edit", title, movie.toJsonObject()));
	}

	@Override
	public boolean remove(String title) throws IOException {
		return isSuccessful(makeJsonRpcRequest("remove", title));
	}
